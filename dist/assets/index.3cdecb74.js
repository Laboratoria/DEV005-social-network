...
function initializeAuth(app, deps) {
  const provider2 = _getProvider(app, "auth");
 ...
 * limitations under the License.
 */
function signInWithRedirect(auth2, provider2, resolver) {
  return _signInWithRedirect(auth2, provider2, resolver);
}
async function _signInWithRedirect(auth2, provider2, resolver) {
  const authInternal = _castAuth(auth2);
  _assertInstanceOf(auth2, provider2, FederatedAuthProvider);
  await authInternal._initializationPromise;
  const resolverInternal = _withDefaultResolver(authInternal, resolver);
  await _setPendingRedirectStatus(resolverInternal, authInternal);
  return resolverInternal._openRedirect(authInternal, provider2, "signInViaRedirect");
}
...
function labelMovement(input, label) {
...
const auth$5 = getAuth(firebaseApp);
const loginApp = (email, password, loginError) => {
  signInWithEmailAndPassword(auth$5, email.value, password.value).then(() => {
  }).catch((error2) => {
    password.value = "";
    if (error2.code === "auth/wrong-password") {
      loginError.textContent = "Contrase\xF1a incorrecta";
    } else if (error2.code === "auth/user-not-found") {
      loginError.textContent = "El usuario no existe";
    } else if (error2.code === "auth/invalid-email") {
      loginError.textContent = "Correo inv\xE1lido";
      ...
      loginError.textContent = "Ingrese una contrase\xF1a";
    } else {
      loginError.textContent = "Ingresa los datos requeridos";
    }
  });
};
const auth$4 = getAuth(firebaseApp);
const provider = new GoogleAuthProvider();
const signInGoogle = () => {
  signInWithRedirect(auth$4, provider).then(() => {
  }).catch(() => {
  });
};
...
const loginFormSubmit = (inputEmail, inputPassword, loginError) => {
...
const navigateToSignup = (navigateTo2) => {
...
function login(navigateTo2) {
  ...
  linkText.textContent = "\xBFNo tienes una cuenta?";
  linkContainer.append(linkText);
  const links = document.createElement("a");
  links.classList.add("links");
  links.href = "";
  links.textContent = "Reg\xEDstrate";
  links.addEventListener("click", navigateToSignup(navigateTo2));
  ...
  text.textContent = "Tambi\xE9n puedes";
...
const auth$3 = getAuth(firebaseApp);
const signupApp = (email, password, registerError) => {
  createUserWithEmailAndPassword(auth$3, email.value, password.value).then(() => {
  }).catch((error2) => {
    password.value = "";
    if (error2.code === "auth/wrong-password") {
      registerError.textContent = "Contrase\xF1a incorrecta";
    } else if (error2.code === "auth/email-already-in-use") {
      registerError.textContent = "El correo ya est\xE1 en uso";
    } else if (error2.code === "auth/invalid-email") {
      registerError.textContent = "Correo inv\xE1lido";
    } else if (error2.code === "auth/too-many-requests") {
      registerError.textContent = "Demasiados intentos";
    } else if (error2.code === "auth/missing-password") {
      registerError.textContent = "Ingrese una contrase\xF1a";
    } else if (error2.code === "auth/weak-password") {
      registerError.textContent = "Contrase\xF1a muy d\xE9bil";
...
const signupFormSubmit = (inputEmail, inputPassword, registerError) => {
...
const navigateToLogin = (navigateTo2) => {
  const preventRefresh = (event) => {
    event.preventDefault();
    navigateTo2("/");
  };
...
function signup(navigateTo2) {
  ...
  const linkText = document.createElement("p");
  linkText.classList.add("link-text");
  linkText.textContent = "\xBFYa tienes una cuenta?";
  linkContainer.append(linkText);
  const links = document.createElement("a");
  links.classList.add("links");
  links.href = "";
  links.textContent = "Inicia sesi\xF3n";
  links.addEventListener("click", navigateToLogin(navigateTo2));
  linkText.append(links);
  registerForm.append(
    emailContainer,
    passwordContainer,
...
const auth$2 = getAuth(firebaseApp);
const logoutApp = (navigateTo2) => {
  signOut(auth$2).then(() => {
    navigateTo2("/");
  }).catch(() => {
  });
...
const savePost = (textPost) => {
  ...
  uf(sh(wh(), "posts"), {
    text,
    author: auth2.currentUser.email,
    likes: []
  });
};
const onGetPost = (callback) => cf(sh(wh(), "posts"), callback);
const deletePost = (id2) => of(rh(wh(), "posts", id2));
const getOnePost = (id2) => Jl(rh(wh(), "posts", id2));
const editPost = (id2, newFields) => rf(rh(wh(), "posts", id2), newFields);
const updateLike = (id2, uid) => rf(rh(wh(), "posts", id2), {
  likes: Sf(uid)
});
const removeLike = (id2, uid) => rf(rh(wh(), "posts", id2), {
  likes: Df(uid)
});
...
const likeAPost = (postId, numLikes, heartLike) => {
  const handleLikeAPost = async (event) => {
    ...
    await getOnePost(postId).then(async (doc) => {
      ...
      const currentUserEmail = auth$1.currentUser.email;
...
const createModal = (postId, containerEachPost, btnEdit) => {
  const handleCreateModal = (event) => {
    ...
    modalContainer.textContent = "\xBFDeseas eliminar definitivamente?";
    const modalButtons = document.createElement("div");
    modalButtons.classList.add("modal-buttons");
    const btnConfirm = document.createElement("button");
    btnConfirm.classList.add("btn-confirm");
    btnConfirm.textContent = "S\xED";
...
const navigateToLoginAfterLogout = (navigateTo2) => {
  const callLogoutApp = (event) => {
    event.preventDefault();
    logoutApp(navigateTo2);
  };
...
function wall(navigateTo2) {
  ...
  logoutIcon.addEventListener("click", navigateToLoginAfterLogout(navigateTo2));
  ...
  textPost.placeholder = "\xBFQu\xE9 quieres compartir?";
