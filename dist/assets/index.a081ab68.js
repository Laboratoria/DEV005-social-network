...
const db = wh();
const savePost = (txtMascotiemos) => uf(sh(db, "posts"), { txtMascotiemos });
const getPosts = () => tf(sh(db, "posts"));
const loginEmail = (email, password) => {
  signInWithEmailAndPassword(auth, email.value, password.value).then(() => {
  }).catch(() => {
  });
};
const userRegister = (email, password, errorEmail, errorPassword) => {
  createUserWithEmailAndPassword(auth, email, password).then(() => {
    ...
    if (error2.code === "auth/email-already-in-use") {
      ...
      errorPassword.innerHTML = "Contrase\xF1a demasiado debil";
    } else if (error2.code === "auth/missing-password") {
      errorPassword.innerHTML = "Contrase\xF1a requerida";
    } else if (error2.code) {
      errorPassword.innerHTML = "Algo sali\xF3 mal";
    }
  });
};
const exit = () => {
    ...
    <main>
      <div class="contenedorIngreso">
        ...
        <span id="errorUsuario" class="errorUsuario"></span>
          <label>
            <span>Correo electr\xF3nico</span>
            <input type="email" autocomplete="off" name="email" class="email" id="email">
          </label>
          <label>
            <span>Contrase\xF1a</span>
            <input type="password" class="password" id="password">
          </label>  
        </form>   
        <button class="btnLogin" id="btnLogin">Iniciar sesi\xF3n</button>
        ...
        <span class="textRegistro">Si no tienes cuenta <button class="btnRegistro" id="btnRegistro">Reg\xEDstrate</button></span>
      </div>
    </main>
  </div>`;
  ...
  <div class='contenedor'>
    ...
    <div class="contenedorIngresoRegistro">
      ...
      <form class="contenedorInput">
        ...
        <span class="textRegistro">Si ya tienes cuenta -> <button class="btnRegistro" id="btnIniciaSesion">Inicia sesi\xF3n</button></span>   
    </div>
    </main>
  </div>`;
...
function home() {
  ...
  <main>
    <header class="contenedorLogoHome">
      ...
      <button name="logOut" id="logOut" class="logOut">Cerrar sesi\xF3n</button>
    </header>
...
function navigation(hash) {
  const route = routes.find((route2) => route2.path === hash);
