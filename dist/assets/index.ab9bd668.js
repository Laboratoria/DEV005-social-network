...
const newAccount = (email, password, errorElement) => {
  if (email === "" && password === "") {
    errorElement.textContent = "Ingrese correo electr\xF3nico y contrase\xF1a";
  } else if (email === "") {
    errorElement.textContent = "Ingrese un correo electr\xF3nico";
  } else if (password === "") {
    errorElement.textContent = "Ingrese una contrase\xF1a.";
  } else if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) {
    errorElement.textContent = "Ingrese un correo v\xE1lido.";
  } else {
    createUserWithEmailAndPassword(auth, email, password).then((userCredential) => {
      ...
      if (errorCode === "auth/email-already-in-use") {
        errorElement.textContent = "El correo ya est\xE1 en uso.";
      } else if (errorCode === "auth/invalid-password") {
        errorElement.textContent = "La contrase\xF1a debe tener al menos 6 caracteres";
      } else {
        errorElement.textContent = "Ocurri\xF3 un error durante el registro.";
      }
    });
  }
};
const logInWithEmail = (email, password, errorELogin) => new Promise((resolve, reject) => {
  if (email === "" && password === "") {
    errorELogin.textContent = "Ingrese correo electr\xF3nico y contrase\xF1a";
    ...
    signInWithEmailAndPassword(auth, email, password).then((userCredential) => {
      ...
      if (errorCode === "auth/user-not-found") {
        errorELogin.textContent = "El correo no est\xE1 registrado.";
      } else if (errorCode === "auth/wrong-password") {
        errorELogin.textContent = "Contrase\xF1a incorrecta.";
      } else if (errorCode === "auth/invalid-email") {
        errorELogin.textContent = "Ingrese un correo electr\xF3nico v\xE1lido.";
      } else {
        errorELogin.textContent = "Ocurri\xF3 un error al intentar iniciar sesi\xF3n.";
      }
      reject(new Error(errorMessage));
    });
  }
});
const accessWithGoogle = (navigateTo2) => {
...
const accessWithGithub = (navigateTo2) => {
  ...
  signInWithPopup(auth, provider).then((result) => {
    ...
    error2.message;
    error2.email;
    GithubAuthProvider.credentialFromError(error2);
...
function home(navigateTo2) {
  ...
  googleLogIn.textContent = "Inicia sesi\xF3n con Google";
  ...
  ghLogIn.textContent = "Inicia sesi\xF3n con GitHub";
  const regist = document.createElement("section");
  regist.id = "to-regist";
  regist.textContent = "\xBFNo tienes cuenta?";
  ...
  infoTxt.textContent = `\xBFTienes un gatito? \xA1En KittyBook puedes presumirlo!
  Publica ideas e im\xE1genes para compartir informaci\xF3n gatuna.`;
  btnLogIn.addEventListener("click", () => {
    const mail = email.value;
    const passwrd = password.value;
    logInWithEmail(mail, passwrd, errorELogin).then(() => {
      navigateTo2("/wall");
    }).catch((error2) => {
      console.log(error2);
    });
  ...
  googleLogIn.addEventListener("click", () => {
    accessWithGoogle(navigateTo2);
  });
  ghLogIn.addEventListener("click", () => {
    accessWithGithub(navigateTo2);
  });
...
function register(navigateTo2) {
  ...
  inputPass.id = "contrase\xF1a";
  inputPass.placeholder = "Escribe tu contrase\xF1a";
  ...
  titleReg.textContent = "\xA1Reg\xEDstrate!";
  titleReg.classList.add("title-registrate");
  btnSend.textContent = "Registrarme";
  btnRegWithGoogle.textContent = "Reg\xEDstrate con Google";
  btnRegWithGithub.textContent = "Reg\xEDstrate con GitHub";
...
const addPostToFirestore = async (text, user) => {
  const userIn = auth.currentUser;
  console.log("se a\xF1ade");
  try {
    const docRef = await uf(sh(db, "posts"), {
      text,
      user: userIn.email,
      likes: [],
      timestamp: Vf()
...
const q = dl(sh(db, "posts"), Il("timestamp", "desc"));
const deletePostFromFirestore = (id2) => of(rh(db, "posts", id2));
const likePost = (userEmail, idDoc) => rf(rh(db, "posts", idDoc), { likes: Sf(userEmail) });
const dislikePost = (userEmail, idDoc) => rf(rh(db, "posts", idDoc), { likes: Df(userEmail) });
function updatePostFirestore(idDoc, newText) {
  const docRef = rh(db, "posts", idDoc);
  rf(docRef, { text: newText }).then(() => console.log("Post updated successfully")).catch((error2) => console.error("Error updating post:", error2));
}
function wall() {
  ...
  modalText.textContent = "Esta publicaci\xF3n ser\xE1 eliminada";
  ...
  btnLogOut.addEventListener("click", () => {
    signOut(auth);
    console.log("Se cerr\xF3 sesi\xF3n correctamente");
  });
  cf(q, (querySnapshot) => {
